...

{
    ...
    if (now - time_ > CLOCKS_PER_SEC * 0.1) {
        printf("\rMultiplying %u polynomials... %u", num_polynomials_to_multiply, num_polynomials_multiplied);
        fflush(stdout);
        time_ = now;
    }
}
...
void 
square_polynomial_karatsuba (
    mpz_t* r, // Assumed to be filled with zeros.
    mpz_t* p, 
    unsigned int l
    ) 

{
    ...
    unsigned int m = l / 2;
    unsigned int m2 = l - m; // m2 is the larger half.

    mpz_t* z1 = malloc(2 * m2 * sizeof(mpz_t));
    mpz_t* z2 = malloc(2 * m2 * sizeof(mpz_t));
    mpz_t* sum_p = malloc(m2 * sizeof(mpz_t));

    for(unsigned int i = 0; i < 2 * m2; i++) {
        mpz_init(z1[i]);
        mpz_init(z2[i]);
        if (i < m2) {
            mpz_init(sum_p[i]);
    ...
    // Prepare P0 + P1 for Z1.
    for (unsigned int i = 0; i < m2; i++) {
        if (i < m) {
            mpz_add(sum_p[i], p[i], p[i + m]);
        } 
        else {
            mpz_set(sum_p[i], p[i + m]);
        }
    }

    // Recursive calls.
    square_polynomial_karatsuba(r, p, m);              // Z0 = P0^2
    square_polynomial_karatsuba(z2, p + m, m2);        // Z2 = P1^2
    square_polynomial_karatsuba(z1, sum_p, m2);        // Z1 = (P0 + P1)^2

    // Calculate Z1 = Z1 - Z0 - Z2.
    for (unsigned int i = 0; i < 2 * m2 - 1; i++) {
        if (i < 2 * m - 1) {
            mpz_sub(z1[i], z1[i], r[i]);
        }
        mpz_sub(z1[i], z1[i], z2[i]);
    }

    // Combine everything into 'r'.
    for (unsigned int i = 0; i < 2 * m2 - 1; i++) {
        mpz_add(r[i + m], r[i + m], z1[i]);
        mpz_add(r[i + 2 * m], r[i + 2 * m], z2[i]);
    }
...
{
    mpz_t* r = malloc(2 * t * sizeof(mpz_t));
    for (unsigned int i = 0; i < 2 * t; i++) {
        mpz_init(r[i]);
    }

    square_polynomial_karatsuba(r, pp->p, t);

    for (unsigned int i = 0; i < t; i++) {
        mpz_set_ui(pp->p_[i], 0);
    }
    for (unsigned int i = 0; i < 2 * t - 1; i++) {
        mpz_add(pp->p_[i % t], pp->p_[i % t], r[i]);
    }
...

{
    ...
    if (n == 0) {
        printf("Usage: %s <N> <M> [--max <M_max>] [--verbose]\n", argv[0]);
    }
    ...
    mpz_t* p1 = malloc(max_t * sizeof(mpz_t));
    mpz_t* p2 = malloc(max_t * sizeof(mpz_t));
    ...
    for (m = m_min; m <= m_max; m++) {
        ...
        */
        ...
        if (verbose) {
            printf("\rMultiplying %u polynomials... %.3f sec.%10s\n", num_polynomials_to_multiply, (double) (clock() - start_time) / CLOCKS_PER_SEC, "");
        ...
        while (pl >= cl) {
            ...
            if (mpz_sgn(*a) != 0) {
                for (unsigned int j = 0; j <= cl; j++) {
                    if (c[j] == 1) {
                        mpz_sub(p[pl - cl + j], p[pl - cl + j], *a);
                    }
                    else if (c[j] == -1) {
                        mpz_add(p[pl - cl + j], p[pl - cl + j], *a);
                    }
                    else if (c[j] > 0) {
                        mpz_submul_ui(p[pl - cl + j], *a, c[j]);
                    }
                    else if (c[j] < 0) {
                        mpz_addmul_ui(p[pl - cl + j], *a, -c[j]);
                    }
                }
            }
            pl--;
